...
class Trainer(object):
    def __init__(self, args, noter):
        ...
        self.n_trend_tr, self.n_non_trend_tr = self.trainloader.n_trend, self.trainloader.n_non_trend
        self.n_trend_val, self.n_non_trend_val = self.valloader.n_trend, self.valloader.n_non_trend
        self.n_trend_test, self.n_non_trend_test = self.testloader.n_trend, self.testloader.n_non_trend
    ...
    def run_train(self):
        ...
        for batch in tqdm(self.trainloader, desc='  - training', leave=False):
            loss_rec_batch, loss_jump_batch, xu_t_plus, xi_t_plus, _, _, dxi_batch = self.model(batch, xu_t_plus, xi_t_plus)
            ...
            dxi_total[0] += dxi_batch[0] / self.n_trend_tr
            dxi_total[1] += dxi_batch[1] / self.n_non_trend_tr
        ...
        self.noter.log_train(loss_tr, loss_rec_total, loss_jump_total, dxi_total, time.time() - time_start)
        self.xu_t_plus = xu_t_plus  # detached at last batch
        self.xi_t_plus = xi_t_plus

    def run_valid(self):
        # validating phase
        rank_val, loss_rec_val, loss_jump_val, dxi_val = self.rollout('validating')
        ...
        self.noter.log_valid(loss_val, loss_rec_val, loss_jump_val, recall_val, mrr_val, dxi_val)
        return recall_val, mrr_val, loss_rec_val, dxi_val

    def run_test(self):
        # testing phase
        rank_test, _, _, dxi_test = self.rollout('testing')

        recall_test = cal_recall(rank_test, self.k_metric)
        mrr_test = cal_mrr(rank_test)

        self.noter.log_test(recall_test, mrr_test, dxi_test)
        return [recall_test, mrr_test, *dxi_test]
    ...
    def rollout(self, mode: str):
        ...
        if mode == 'validating':
            dl = self.valloader
            len_dl, n_trend, n_non_trend = self.len_val_dl, self.n_trend_val, self.n_non_trend_val
        else:
            assert mode == 'testing'
            dl = self.testloader
            len_dl, n_trend, n_non_trend = self.len_test_dl, self.n_trend_test, self.n_non_trend_test
        ...
        with torch.no_grad():
            for batch in tqdm(dl, desc='  - ' + mode, leave=False):
                ...
                loss_rec_batch, loss_jump_batch, xu_t_plus, xi_t_plus, xu_enc, xi_enc, dxi_batch = self.model(batch,
                                                                                                              xu_t_plus,
                                                                                                              xi_t_plus)

                loss_rec_total += loss_rec_batch.item() / len_dl
                loss_jump_total += loss_jump_batch.item() / len_dl
                dxi_total[0] += dxi_batch[0] / n_trend
                dxi_total[1] += dxi_batch[1] / n_non_trend
        ...
        return rank_u, loss_rec_total, loss_jump_total, dxi_total
